...
import com.eternaldoom.redstonepack.GUIHandler;
import com.eternaldoom.redstonepack.RedstoneExpansionPack;
...
public class BlockSmelter extends BlockContainer
{
    ...
    {
        return Item.getItemFromBlock(ModBlocks.smelter);
    }
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (this.isBurning)
        {
            ...
            switch (BlockSmelter.SwitchEnumFacing.FACING_LOOKUP[enumfacing.ordinal()])
            {
                case 1:
	...
	public boolean onBlockActivated(World var1, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float var7, float var8, float var9) {
		if (!player.isSneaking()) {
			player.openGui(RedstoneExpansionPack.instance, GUIHandler.smelter, var1, pos.getX(), pos.getY(), pos.getZ());
			return true;
		} else {
			return false;
		}
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos)
    {
        ...
        if (active)
        {
            worldIn.setBlockState(pos, ModBlocks.smelter_on.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
            worldIn.setBlockState(pos, ModBlocks.smelter_on.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
        }
        else
        {
            worldIn.setBlockState(pos, ModBlocks.smelter.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
            worldIn.setBlockState(pos, ModBlocks.smelter.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
        }
    ...
    public BlockSmelter register(String name){
